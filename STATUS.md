...
**已游戏内验证**: P0 全 + P1 step 7-13 + 18.
**未验证**: Phase 1-3 改动 (要下一把游戏跑)
...
### Phase 4 (下一把游戏验证)
- 验 set_doctrine 一句话设两层
- 验 set_objective("harass_economy") 真派 cycle harass + 切目标自动清
- 验 harass 单次行为 (不再吃新单位)
- 跑 P1 step 14-17 + 19 (上把没跑完)

### 未做 TODO (低优)
- `control_map_center` objective 自动派 contain mission (需 map_center 解析)
- `survive_until_tick` 自动行为 (现仅软指引)
