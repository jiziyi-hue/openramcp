...
### Phase 6 (本会话末做的) — 代码 DONE, 待游戏验证
- ✓ 病 D 修: attack mission (所有, 不光 cycle) 默认 named retarget — 派 attack/pincer 时
  interpreter 自动把 target.name 传 register_assault, daemon target 死时自动重查
- ✓ 病 B 修: filter 加 prefer 字段 (strongest/fastest/healthiest/any) + combat_mobile 字段;
  interpreter._force_by_filter 和 tactical._filter_force_ids_from_world 都按 prefer sort
- ✓ 病 F 修: daemon 每 30s push mission_progress 事件 (per active assault/harass) 含
  force_alive, avg_hp_pct, distance_to_target, target_alive, target_named
- ✓ Doctrine 表重写 (db395b9): unit > 建筑, glass-cannon(v2rl/arty/dtrk)=100 最顶,
  e3 anti-armor 90, harv 85, 重坦 65-80, 建筑里 powr 最高 75-78 (chain effect)
- ✓ extended-range chase (radius 18) 仅对 priority>=80 高威胁
- ✓ 4 prompt 文件更新 — destroy_enemy / set_doctrine / prefer / mission_progress 用法

### 未做 TODO
- `control_map_center` objective 自动派 contain mission
- `survive_until_tick` 自动切 lockdown + watchdog
- 上把没跑完的 P1 step 14-17 + 19
- "release_unit" — 玩家 UI 单独抢回某单位 (病 A 未修)
- target memory (病 C 未修)
- kill zone 感知 (病 E 未修)
- **strategy_phase_advice 自动战略建议器** (用户提议 2026-05-23):
  daemon 定期采样 cash/mobile_count/harv/proc/敌方接近, 按规则判阶段
  (early/mid/threat/harass_ready/push_ready/endgame), push 事件 LLM 转告
  玩家. 类似 alert_state_suggestion 扩展. 设计 ~150 行, 简版 ~80 行.
- **`combat_mobile` filter 太宽** — 把 v2rl 等慢速攻城炮也收编, daemon
  推进时自动派 v2 不合理 (v2 应留后排手控). 加 exclusion 列或拆
  `combat_pushable` (3tnk/4tnk/2tnk/apc/e1/e3 等) vs `combat_mobile`
