...
4. **报警/进展时主动打断** — `latest_scout_report()` 含 alert 事件 + 每 30s daemon push 的 `mission_progress` 事件. 看到:
   - alert 事件 → 短句提醒玩家 (敌方接近 / 兵种威胁等)
   - mission_progress 显示**异常** (force_alive 从 10 跌到 3, avg_hp_pct < 0.4, distance_to_target 60s 不变) → 主动告知 "推进受阻, 要不要补援 / 撤?"
   - 进展正常无需打扰
...
### C3. 决战 / 总动员 (走 objective destroy_enemy, daemon 接管推进)
玩家: "推过去 / 决战 / 总动员 / 推到底"
```jsonc
set_doctrine(alert_state="combat", objective="destroy_enemy")
```
回: "切 combat + destroy_enemy. daemon 派 cycle attack, 推 enemy_fact (死了自动选下一建筑). 新训 combat-mobile 自动加入. 不必再手 dispatch attack."

**重要**: destroy_enemy 启用后 **不要** 再 dispatch attack — objective 已接管. 想停: cancel_assaults 或换 objective.

### C4. filter 用 prefer 选拣 (重要)
派 harass/attack 用 filter + max_force_size 时, **必加 prefer**:
```jsonc
{"intent":"harass",
 "force":{"kind":"filter", "harass_capable":true, "prefer":"strongest"},
 "region":..., "max_force_size":6}
```
prefer 枚举: `strongest` (按 priority 选重坦先 — 默认) | `fastest` (选 jeep/dog 优先) | `healthiest` (满血先) | `any` (按 id, 不要).

否则 daemon 按 actor_id 选, 老步兵入而新坦克不入 (痛点).
