# === OpenRA Qwen-0.5B v6 (9-intent+pincer command surface) — train + eval in Colab ===
...
model.save_pretrained('qwen05b_openra_lora_v6'); tok.save_pretrained('qwen05b_openra_lora_v6')
import shutil
shutil.make_archive('qwen05b_openra_lora_v6','zip','qwen05b_openra_lora_v6')
try:
    r=subprocess.check_output(['curl','-sfL','-F','file=@qwen05b_openra_lora_v6.zip','https://tmpfiles.org/api/v1/upload'],timeout=180).decode()
    print('TMPFILES_URL:', json.loads(r)['data']['url'])
except Exception as e: print('tmpfiles failed:', str(e)[:60])
from google.colab import files
files.download('qwen05b_openra_lora_v6.zip')
print('ALL DONE')
